...
- 桌面 portable 发布包当前默认采用“干净发布包”策略，不再携带开发者个人 `resume-package/` 数据。
- 桌面首包默认也不再分发 `interview-kit/templates/`，模板资产在桌面主线中以用户自行创建、保存或导入的数据为准。
- 这不会改变主库建模方向；它影响的是“默认资产来源”与“首包初始化策略”，而不是数据库作为业务真相源的定位。
- 因此后续实现仍应坚持：模板、题库、review、knowledge、runtime read model 统一以数据库主路径为核心，只把发布包内置数据控制在必要且中性的最小集合。
