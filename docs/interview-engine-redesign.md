...
引擎层设计文档、默认提示词示例、模板占位内容和调试演示数据，应优先使用泛化且不可识别的中性描述。

- 不默认写入真实公司、产品、人员、地点或可回溯到具体业务实体的案例。
- 如果需要示例，应表达结构、字段关系和交互意图，而不是复用真实招聘或真实简历内容。
- 这条原则同样适用于桌面发布包、默认模板、空状态文案和调试视图示例，避免实现与文档在内容合规上出现偏差。
